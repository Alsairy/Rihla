...
class MockDataService {
  ...
  getStudents(page: number = 1, pageSize: number = 10): Promise<{ data: Student[], total: number }> {
  ...
  getDrivers(page: number = 1, pageSize: number = 10): Promise<{ data: Driver[], total: number }> {
    ...
    for (let i = 0; i < pageSize; i++) {
      ...
      drivers.push({
        ...
        licenseExpiryDate: this.generateDate(-Math.floor(Math.random() * 365) + 365),
        ...
        vehicleId: Math.random() > 0.2 ? Math.floor(Math.random() * 38) + 1 : undefined,
        tenantId: 1,
        createdAt: this.generateDate(Math.floor(Math.random() * 365)),
        updatedAt: this.generateDate(Math.floor(Math.random() * 30))
      });
  ...
  getVehicles(page: number = 1, pageSize: number = 10): Promise<{ data: Vehicle[], total: number }> {
    ...
    for (let i = 0; i < pageSize; i++) {
      ...
      vehicles.push({
        ...
        insuranceExpiryDate: this.generateDate(-Math.floor(Math.random() * 365) + 365),
        registrationExpiryDate: this.generateDate(-Math.floor(Math.random() * 365) + 365),
        lastMaintenanceDate: this.generateDate(Math.floor(Math.random() * 90)),
        nextMaintenanceDate: this.generateDate(-Math.floor(Math.random() * 30) + 30),
        isActive: Math.random() > 0.05,
        driverId: Math.random() > 0.2 ? Math.floor(Math.random() * 45) + 1 : undefined,
        driverName: Math.random() > 0.2 ? `Driver ${Math.floor(Math.random() * 45) + 1}` : undefined,
        currentDriverId: Math.random() > 0.2 ? Math.floor(Math.random() * 45) + 1 : undefined,
        tenantId: 1,
        createdAt: this.generateDate(Math.floor(Math.random() * 365)),
        updatedAt: this.generateDate(Math.floor(Math.random() * 30))
      });
  ...
  getRoutes(page: number = 1, pageSize: number = 10): Promise<{ data: Route[], total: number }> {
    const routes: Route[] = [];
    const total = 150;
    const areas = ['Al-Malaz', 'Al-Olaya', 'Al-Sulaimaniyah', 'King Fahd', 'Al-Nakheel', 'Al-Wurud'];
    ...
    for (let i = 0; i < pageSize; i++) {
      ...
      routes.push({
        ...
        vehicleId: Math.random() > 0.2 ? Math.floor(Math.random() * 38) + 1 : undefined,
        driverId: Math.random() > 0.2 ? Math.floor(Math.random() * 45) + 1 : undefined,
        assignedVehicleId: Math.random() > 0.2 ? Math.floor(Math.random() * 38) + 1 : undefined,
        assignedDriverId: Math.random() > 0.2 ? Math.floor(Math.random() * 45) + 1 : undefined,
        tenantId: 1,
        createdAt: this.generateDate(Math.floor(Math.random() * 365)),
        updatedAt: this.generateDate(Math.floor(Math.random() * 30))
      });
  ...
  getTrips(page: number = 1, pageSize: number = 10): Promise<{ data: Trip[], total: number }> {
    const trips: Trip[] = [];
    const total = 500;
    ...
    for (let i = 0; i < pageSize; i++) {
      ...
      scheduledTime.setHours(7 + Math.floor(Math.random() * 8), Math.floor(Math.random() * 60));
      ...
      trips.push({
        ...
        actualStartTime: Math.random() > 0.3 ? scheduledTime.toISOString() : undefined,
        scheduledEndTime: new Date(scheduledTime.getTime() + 45 * 60000).toISOString(),
        actualEndTime: Math.random() > 0.5 ? new Date(scheduledTime.getTime() + 45 * 60000).toISOString() : undefined,
  ...
  getAttendance(page: number = 1, pageSize: number = 10): Promise<{ data: Attendance[], total: number }> {
    ...
    for (let i = 0; i < pageSize; i++) {
      ...
      attendance.push({
        ...
        notes: Math.random() > 0.8 ? `Attendance note for record ${id}` : undefined,
        tenantId: 1,
        createdAt: this.generateDate(Math.floor(Math.random() * 30)),
        updatedAt: this.generateDate(Math.floor(Math.random() * 7))
      });
  ...
  getPayments(page: number = 1, pageSize: number = 10): Promise<{ data: Payment[], total: number }> {
    ...
    for (let i = 0; i < pageSize; i++) {
      ...
      payments.push({
        ...
        paidDate: Math.random() > 0.3 ? this.generateDate(Math.floor(Math.random() * 30)) : undefined,
        status: statuses[Math.floor(Math.random() * statuses.length)],
        paymentMethod: Math.random() > 0.3 ? methods[Math.floor(Math.random() * methods.length)] : undefined,
        transactionId: Math.random() > 0.3 ? `TXN${id.toString().padStart(8, '0')}` : undefined,
  ...
  getMaintenanceRecords(page: number = 1, pageSize: number = 10): Promise<{ data: MaintenanceRecord[], total: number }> {
    ...
    for (let i = 0; i < pageSize; i++) {
      ...
      records.push({
        ...
        completedDate: Math.random() > 0.4 ? this.generateDate(Math.floor(Math.random() * 15)) : undefined,
        cost,
        status: statuses[Math.floor(Math.random() * statuses.length)],
        serviceProvider: `Service Provider ${Math.floor(Math.random() * 10) + 1}`,
        notes: Math.random() > 0.6 ? `Additional notes for maintenance ${id}` : undefined,
        tenantId: 1,
  ...
  getNotifications(page: number = 1, pageSize: number = 10): Promise<{ data: Notification[], total: number }> {
  ...
  updateStudent(id: number, student: Partial<Student>): Promise<Student> {
    return this.getStudents(1, 1).then(result => {
      ...
      const updated = { ...existing, ...student, id, updatedAt: new Date().toISOString() };
      return updated;
    });
  }
  ...
  login(email: string, password: string): Promise<{ user: User; token: string }> {
    const mockUsers = [
      { id: 1, email: 'admin@rihla.com', username: 'admin', role: 'Admin', tenantId: 1 },
      { id: 2, email: 'parent@rihla.com', username: 'parent', role: 'Parent', tenantId: 1 },
      { id: 3, email: 'driver@rihla.com', username: 'driver', role: 'Driver', tenantId: 1 }
    ];
  ...
  getVehicleLocation(vehicleId: number): Promise<{ lat: number; lng: number; timestamp: string }> {
    return Promise.resolve({
