...
const VehicleManagementDashboard: React.FC<VehicleManagementDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    const setupRealTimeUpdates = () => {
      signalRService.onVehicleStatusChanged(() => {
        fetchVehicleData();
      });
  ...
  const fetchVehicleData = async () => {
    ...
    try {
      ...
      setMaintenanceAlerts(
        Array.isArray(maintenanceAlertsResponse)
          ? maintenanceAlertsResponse
          : []
      );
      setInsuranceAlerts(
        Array.isArray(insuranceAlertsResponse) ? insuranceAlertsResponse : []
      );
  ...
  const calculateFleetStats = (vehiclesData: Vehicle[]) => {
    ...
    const stats = {
      ...
      inMaintenance: vehiclesData.filter(v => v.status === 'Maintenance')
        .length,
      outOfService: vehiclesData.filter(v => v.status === 'OutOfService')
        .length,
      averageAge:
        vehiclesData.length > 0
          ? Math.round(
              vehiclesData.reduce((sum, v) => sum + (currentYear - v.year), 0) /
                vehiclesData.length
            )
          : 0,
  ...
  const uniqueFuelTypes = [
    'All',
    ...Array.from(new Set(vehicles.map(v => v.fuelType))),
  ];
                  ...
                  {maintenanceAlerts.slice(0, 5).map(alert => (
                    <ListItem key={alert.id} sx={{ px: 0 }}>
                      ...
                      <Button
                        ...
                        variant="outlined"
                        onClick={() =>
                          openMaintenanceScheduler(alert.vehicleId)
                        }
                      >
                        Schedule
                      </Button>
          ...
          <Box sx={{ flex: '1 1 400px', minWidth: '300px' }}>
            ...
            >
              <CardContent>
                ...
                <List sx={{ maxHeight: 300, overflow: 'auto' }}>
                  {insuranceAlerts.slice(0, 5).map(alert => (
                    <ListItem key={alert.id} sx={{ px: 0 }}>
                      <ListItemIcon>
                        <SecurityIcon
                          color={
                            alert.daysUntilExpiry <= 30 ? 'error' : 'warning'
                          }
                        />
                      </ListItemIcon>
                      <ListItemText
                      ...
                      <Chip
                        label={
                          alert.daysUntilExpiry <= 30 ? 'Critical' : 'Warning'
                        }
                        color={
                          alert.daysUntilExpiry <= 30 ? 'error' : 'warning'
                        }
                  ...
                  <TableRow key={vehicle.id} hover>
                    ...
                    <TableCell>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {vehicle.year} • {vehicle.color} • {vehicle.capacity}{' '}
                        seats
                      </Typography>
                    </TableCell>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        ...
                        <Tooltip title="Maintenance">
                          <IconButton
                            size="small"
                            onClick={() => openMaintenanceScheduler(vehicle.id)}
                          >
