...
const MapComponent: React.FC<MapComponentProps> = ({
  ...
  const loadMapData = async () => {
    try {
      ...
      if (showVehicles) {
        promises.push(
          apiClient.get<{ data: Vehicle[]; total: number }>('/api/vehicles')
        );
      }

      if (showRoutes) {
        promises.push(
          apiClient.get<{ data: Route[]; total: number }>('/api/routes')
        );
      }

      if (showStudents) {
        promises.push(
          apiClient.get<{ data: Student[]; total: number }>('/api/students')
        );
      }
      ...
      let resultIndex = 0;
      if (showVehicles) {
        const vehiclesData = results[resultIndex]?.data || [];
        setVehicles(vehiclesData);
        resultIndex++;
      }
      if (showRoutes) {
        const routesData = results[resultIndex]?.data || [];
        setRoutes(routesData);
        resultIndex++;
      }
      if (showStudents) {
        const studentsData = results[resultIndex]?.data || [];
