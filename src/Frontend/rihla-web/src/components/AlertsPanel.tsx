...
const AlertsPanel: React.FC<AlertsPanelProps> = ({
  ...
  const alertCategories = {
    critical: {
      ...
      filter: (alert: AlertItem) =>
        alert.severity === 'error' || alert.daysUntilDue <= 0,
    },
    warning: {
      label: 'Warnings',
      icon: <WarningIcon />,
  ...
  const fetchAlerts = useCallback(async () => {
    try {
      ...
      const driverCertificationResponse = (await apiClient.get(
        '/api/drivers/certification-alerts'
      )) as any[];
      ...
      const vehicleMaintenanceResponse = (await apiClient.get(
        '/api/vehicles/maintenance-alerts'
      )) as any[];
      ...
      const vehicleInsuranceResponse = (await apiClient.get(
        '/api/vehicles/insurance-alerts'
      )) as any[];
      ...
      const vehicleRegistrationResponse = (await apiClient.get(
        '/api/vehicles/registration-alerts'
      )) as any[];
  ...
  const getAlertIcon = (alert: AlertItem) => {
    switch (alert.type) {
      case 'driver_certification':
        if (alert.category.toLowerCase().includes('license'))
          return <LicenseIcon />;
        if (alert.category.toLowerCase().includes('medical'))
          return <MedicalIcon />;
        if (alert.category.toLowerCase().includes('training'))
          return <TrainingIcon />;
        return <DriverIcon />;
  ...
  const getAlertsByCategory = (categoryKey: string) => {
    const category =
      alertCategories[categoryKey as keyof typeof alertCategories];
  ...
  return (
    <Paper sx={{ p: 3 }}>
      ...
      <TabPanel value={activeTab} index={2}>
        {renderAlertsList(
          alerts.filter(alert => alert.type.startsWith('vehicle_'))
        )}
      </TabPanel>
